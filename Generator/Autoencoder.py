...
from Generator.Delenox_Config import lattice_dimensions, batch_size, no_epochs, thread_count, value_range, current_run, \
    compressed_length
from Generator.Visualization import auto_encoder_plot, visualize_training
...
def create_auto_encoder(model_type, phase, experiment, population=None, noisy=None, save=True):
    ...
    if save:
        if phase == -1:
            save_model(encoder_model, "./Results/Seed/{}".format(encoder_name))
            save_model(decoder_model, "./Results/Seed/{}".format(decoder_name))
        else:
            save_model(encoder_model, "./Results/{}/Phase{:d}/encoder".format(experiment, phase))
            save_model(decoder_model, "./Results/{}/Phase{:d}/decoder".format(experiment, phase))
